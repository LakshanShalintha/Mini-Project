                            ...
                            return FutureBuilder<Reference>(
                              ...
                              builder: (context, snapshot) {
                                if (snapshot.connectionState ==
                                  ...
                                  final fileName =
                                      fileRef.name.split('.pdf')[0];
                                  return GestureDetector(
                                    onTap: () {
                                      Navigator.of(context).push(
                                        MaterialPageRoute(
                                          builder: (context) =>
                                              PDFViewerScreen(fileRef: fileRef),
                                        ),
                                      );
                                    },
                                    child: Card(
  ...
  Future<void> loadFavorites() async {
    ...
    if (user != null) {
      DocumentSnapshot snapshot =
          await _firestore.collection(_favoritesCollection).doc(user.uid).get();

      if (snapshot.exists) {
        _favoritePdfUrls = List<String>.from(
            (snapshot.data() as Map<String, dynamic>)['pdfUrls'] ?? []);
      }
    }
  }
